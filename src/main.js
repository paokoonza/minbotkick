...
var myBott = ['ua26d317bf6c058a4c00aa93caee1ab0d'];//TARO MID LU DISINI SUPAYA BISA PKE COMMAND STAFF

const myBot = ['ua1cb6e845fe8f2646fe8a5c5911841fa'];//TARO MID LU DISINI
...
class LINE extends LineAPI {
	    ...
	    this.textMessage("0103",seq,operation.param2,1);
          ...
          if(operation.type == 15 && this.stateStatus.bmsg == 1) {
           ...
           if(operation.type == 16 && this.stateStatus.bmsg == 1) {
             ...
             itil.text = "[SELFBOT MIN HACK BOT]\n[ขอบคุณสำหรับคำเชิญ]\n\n[By.มิน ทดลองบอท]"
    ...
    async textMessage(textMessages, seq, param, lockt) {
      ...
      if(txt == 'tab:add:staff' && this.sendStaff == 0 && isAdminOrBot(seq.from)){
         this.sendStaff = 1;
       ...
       if(seq.contentType == 13 && this.sendStaff == 1 && isAdminOrBot(seq.from)) {
        ...
        if(txt == 'tab:del:staff' && this.sendStaff == 0 && isAdminOrBot(seq.from))
{
           this.sendStaff = 2;
           ...
           if(seq.contentType == 13 && this.sendStaff == 2 && isAdminOrBot(seq.from))
{
    ...
    await this._sendMessage(seq,'Sukses Menghapus Staff~');
        ...
        if(txt == 'sp') {
            const curTime = (Date.now() / 3000000);

            await this._sendMessage(seq,'รอหน่อยหำ...');


            const rtime = (Date.now() / 3000000) - curTime;
            await this._sendMessage(seq, `${rtime} ได้เท่านี้แหละ`);
        }
...
let { listMember } = await this.searchGroup(seq.to);
     const mentions = await this.mention(listMember);
        ...
        if(txt === 'บิน.' && this.stateStatus.kick == 1 && isStaffOrBot(seq.from)) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        if(txt == 'me') {
           this._sendMessage(seq,'มีหรือป่าว..');
           seq.contentType=13;
           seq.contentMetadata = { mid: 'ua1cb6e845fe8f2646fe8a5c5911841fa' };
           this._client.sendMessage(1, seq);
        }
